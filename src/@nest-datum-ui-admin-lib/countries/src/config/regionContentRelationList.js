...
const regionContentRelationList = {
	title: 'Values',
	...
	apiMainModelUrl: `${process.env.URL_API_COUNTRIES}/region`,
	...
	rowColumns: [{
		...
		wrapper: (item, { id }) => <TypographyTable 
			onClick={() => actionDialogOpen('relation', { regionContentId: id })()} 
			sx={{ 
				cursor: 'pointer', 
			}}>
			{String(id)}
		...
		id: 'value',
		order: 2,
		wrapper: (item, { typeOptionId, value }) => <FormsTypographyOptionValue 
...
export default regionContentRelationList;
